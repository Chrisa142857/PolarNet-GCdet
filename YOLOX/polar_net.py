...
class PolarNet(torch.nn.Module):
    def __init__(self, mtype='v3', hidden_num=768, feat_num=2048, max_possible_img_size=1024/32):
        super().__init__()
...
def get_mask(polarity, strides, boxes, feat_k):
    nlabel = (boxes.sum(dim=2) > 0).sum(dim=1)  # number of objects
    
    fg_masks = []
    bg_masks = []
    hsize, wsize = polarity.shape[-2:]
    dtype = polarity.type()
    stride = strides[feat_k]
    for batch_idx in range(boxes.shape[0]):
        ...
        if num_gt == 0:
            ...
            continue
        box = boxes[batch_idx, :num_gt, 1:5]
        yv, xv = torch.meshgrid([torch.arange(hsize), torch.arange(wsize)])
        grid = torch.stack((xv, yv), 2).view(1, 1, hsize, wsize, 2).type(dtype)
        grid = grid.view(1, -1, 2)
        grid = grid * stride + stride/2
        grid_x = grid[:, :, 0].repeat(num_gt, 1) # num_gt x num_grid
        grid_y = grid[:, :, 1].repeat(num_gt, 1) # num_gt x num_grid

        box_x1 = (box[:, 0:1]-box[:, 2:3]/2).repeat(1, hsize*wsize) # num_gt x num_grid
        box_y1 = (box[:, 1:2]-box[:, 3:4]/2).repeat(1, hsize*wsize) # num_gt x num_grid
        box_x2 = (box[:, 0:1]+box[:, 2:3]/2).repeat(1, hsize*wsize) # num_gt x num_grid
        box_y2 = (box[:, 1:2]+box[:, 3:4]/2).repeat(1, hsize*wsize) # num_gt x num_grid
