...
class OODDataset(object):
    ...
    def __getitem__(self, index):
        ...
        if tile.shape[-1] != self.input_side or tile.shape[-2] != self.input_side:
...
class StructuredPool(torch.nn.Module):
    def __init__(self, gsize=13, tshape=(8, 8)):
    ...
    def build_mask(self, torch_imgs):
        ...
        for i in range(self.mask.shape[0]):
            ...
            sub_img = np.clip(sub_img, 0, sub_img.max())
            self.mask[i] = torch.from_numpy(sub_img)

    def forward(self, x):
        mask = torch.nn.functional.interpolate(self.mask.unsqueeze(1), size=self.tshape)
        mask = self.flatten(mask[:, 0, :, :])
        mask = self.fc(mask)
