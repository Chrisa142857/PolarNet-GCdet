...
class Exp(MyExp):
    ...
    def get_model(self, postproc=False, **kwargs):
        ...
        from backbones import Res50, ViTbackbone
        ...
        if getattr(self, "model", None) is None:
            # backbone = Res50(pretrained=True)
            backbone = ViTbackbone(name="B_16", pretrained=True, input_size=1024, input_size_to_transformer=512)
            in_channels = backbone.in_channels
            head = YOLOXHead(self.num_classes, self.width, in_channels=in_channels, act=self.act, strides=(backbone.patch_size,), **kwargs)
            self.model = YOLOX(backbone, head, postproc)

        self.model.apply(init_yolo)
        self.model.head.initialize_biases(1e-2)
        # self.model.backbone.apply(freeze_p)
        return self.model
