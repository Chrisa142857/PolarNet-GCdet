...
class glandular_evaluator:
    ...
    def evaluate(
        ...
        for cur_iter, (imgs, targets, info_imgs, ids) in enumerate(
            ...
            if is_time_record:
                start = time.time()
            outputs = model(imgs, return_pscore=True)
            ...
            if len(bboxes) == 0:
                ...
                # p_scores.append(pscores[new_output_index])
                if len(new_boxes) != 0:
        ...
        torch.save({'p_score': p_scores}, self.cls_csv_path.replace('_cls', '_pscore').replace('.csv', '.pth'))
        info = time_info + "\n"
