...
class glandular_evaluator:
    ...
    def evaluate(
        ...
        for cur_iter, (imgs, targets, info_imgs, ids) in enumerate(
            ...
            if is_time_record:
                start = time.time()
            outputs, fg_scores = model(imgs)

            fg_acc += model.fg_net.squarer.get_batch_acc(fg_scores, targets)
...
class Exp(MyExp):
    ...
    def get_model(self, postproc=False, **kwargs):
        ...
        if getattr(self, "model", None) is None:
            fg_net = ForegroundNet(name="B_16", pretrained=True, input_size=512)
            ...
            self.model = YOLOX(backbone, head, fg_net=fg_net, postprocess=postproc)
