...
class glandular_evaluator:
    ...
    def evaluate(
        ...
        for cur_iter, (imgs, targets, info_imgs, ids) in enumerate(
            ...
            if is_time_record:
                start = time.time()
            outputs, pscores = model(imgs, return_pscore=True)

            if is_time_record:
                infer_end = time_synchronized()
                inference_time += infer_end - start

            outputs, pscores = postprocess(
                outputs, self.num_classes, self.confthre, self.nmsthre, pscores=pscores
            )
            ...
            if outputs[-1] == None:
                locs.append("")
                preds.append(0)
                pscrs.append(torch.empty(0))
                bboxes = []
                cls = []
                scores = []
            else:
                pscore = pscores[-1].cpu()
                output = outputs[-1].cpu()
                bboxes = output[:, 0:4]
                cls = output[:, 6]
                # scores = (output[:, 4] + pscore[0]) * output[:, 5]
            ...
            if len(bboxes) == 0:
                ...
                pscrs.append(pscore[new_output_index])
                if len(new_boxes) != 0:
        ...
        torch.save({'p_score': pscrs}, self.cls_csv_path.replace('_cls', '_pscore').replace('.csv', '.pth'))
...
class Exp(MyExp):
    def __init__(self):
        ...
        # ------------
        polar_net = PolarNet(mtype='v4')
        self.model = self.get_model(depthwise=True, postproc=False, polar_net=polar_net)
    ...
    def get_data_loader(self, batch_size, is_distributed, no_aug=False, cache_img=False, data_type=None):
        ...
        dataset = get_dataset("D:/WSI_analysis/det_train/AttFaster-glandular/train_w_neg.csv",
                              datatype="train",
    ...
    def get_evaluator(self, batch_size, is_distributed, testdev=False, legacy=False):
        ...
        tail = '_polar_w_neg'
        if testdev:
            gt_csv_path='D:/WSI_analysis/det_train/AttFaster-glandular/test_wo_ood_w_neg.csv'
            # gt_csv_path='D:/WSI_analysis/det_train/AttFaster-glandular/test_wo_ood.csv'
            # gt_csv_path='D:/WSI_analysis/det_train/AttFaster-glandular/test_w_ood.csv'
        else:
            gt_csv_path='D:/WSI_analysis/det_train/AttFaster-glandular/val_w_neg.csv'
        ...
        evaluator = glandular_evaluator(
            ...
            cls_csv_path='temp/yolox_dw_m%s_%s_cls.csv'% (tail, gt_csv_path.split('/')[-1]),
            loc_csv_path='temp/yolox_dw_m%s_%s_loc.csv'% (tail, gt_csv_path.split('/')[-1])
        )
        return evaluator
