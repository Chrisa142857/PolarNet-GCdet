...
class CascadeRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward(self, stage, x, rois):
        ...
        if hasattr(self, 'use_polar'):
            if self.use_polar:
                x, self.polar_score = x
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        if hasattr(self, 'use_polar'):
            if self.use_polar:
                for i in range(num_imgs):
                    pred_boxes = [det_bboxes[i][:, :4]]
                    polar_score = roi_align(self.polar_score.permute(0,3,1,2), pred_boxes, output_size=7, spatial_scale=self.polar_scale)
                    if len(polar_score) > 0:
                        polar_score = torch.nn.functional.avg_pool2d(polar_score, (7, 7)).squeeze(-1).squeeze(-1)
                        polar_score = torch.nn.functional.softmax(polar_score)[:, 1]
                        det_bboxes[i][:, 4] = (1-self.polar_alpha)*det_bboxes[i][:, 4] + self.polar_alpha*polar_score
        ####################################################################################
        bbox_results = [
